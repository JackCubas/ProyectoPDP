...
trap 'echo "[ERROR] Línea $LINENO"; exit 1' ERR
...
apt install -y curl git build-essential unzip wget ca-certificates lsb-release gnupg
...
wget -q https://github.com/JackCubas/ProyectoPDP/archive/refs/heads/multiajuste.zip -O proyecto.zip
unzip -q proyecto.zip
PROYECTO="$directorio/ProyectoPDP-multiajuste"
...
EOF
    ...
    $DB_BIN -u root -p"${MYSQL_PASS}" <<EOF
...
EOF
    ...
    $DB_BIN -u root -p"${MYSQL_PASS}" -e "CREATE DATABASE IF NOT EXISTS $DB_NAME;"

    echo "Otorgando privilegios..."
    $DB_BIN -u root -p"${MYSQL_PASS}" -e "GRANT ALL PRIVILEGES ON $DB_NAME.* TO 'root'@'localhost' WITH GRANT OPTION;"

    echo "Importando base de datos..."
    $DB_BIN -u root -p"${MYSQL_PASS}" $DB_NAME < firma_app.sql
    ...
    cat > /etc/systemd/system/proyecto-backend.service <<EOF
[Unit]
...
systemctl enable proyecto-backend
systemctl restart proyecto-backend
...
#log "Preparing MySQL database and user..."
...
#sudo mariadb xcs < /home/vboxuser/xcs-schema.sql
