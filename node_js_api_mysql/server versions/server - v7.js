...
async function genKeyPairSimple() {
...
//Utilidad para insertar la firma:
function insertSignatureIntoPdf(pdfBuffer, signatureHex) {
  ...
  const placeholder = /\/Contents\s*<([A-F0-9]+)>/;
  const match = pdfString.match(placeholder);

  if (!match) {
    throw new Error('No se encontró el placeholder de firma');
  }

  const placeholderHexLength = match[1].length;

  if (signatureHex.length > placeholderHexLength) {
    throw new Error('La firma es más grande que el placeholder');
  }

  const paddedSignature =
    signatureHex + '0'.repeat(placeholderHexLength - signatureHex.length);
...
app.post('/sign/finalize/:id', async (req, res) => {
  ...
  con.connect(async (err) => {
    ...
    //-----------------------------------------------------  
      ...
      // Insertamos la firma
      const signedPdfBuffer = insertSignatureIntoPdf(
        pdfBytes,
        signatureHex
      );
