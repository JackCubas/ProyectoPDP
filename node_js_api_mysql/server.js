...
app.post('/sign/finalize/:id', async (req, res) => {
  ...
  con.connect(async (err) => {
    ...
    if (fs.existsSync("./dump_crt.crt")) { 
      // delete the file on server after it sends to client
      const unlinkFile = util.promisify(fs.unlink); // to del file from local storage
      await unlinkFile("./dump_crt.crt");
    }
    ...
    //cert.setSubject(existingCert.subject.attributes); // Copy subject details
    //cert.setIssuer(existingCert.issuer.attributes);   // Copy issuer details
    ...
    cert.sign(keysForge.privateKey, forge.md.sha256.create());
    var pem = {
        privateKey: forge.pki.privateKeyToPem(keysForge.privateKey),
        publicKey: forge.pki.publicKeyToPem(keysForge.publicKey),
        certificate: forge.pki.certificateToPem(cert)
    };
    var certPem = forge.pki.certificateFromPem(pem.certificate)


    fs.writeFileSync("./dump_pem.pem", pem.certificate);
    fs.writeFileSync("./dump_crt.crt", Buffer.from(JSON.stringify(cert)));

    //console.log('\nKey-Pair:');
    //console.log('br', cert.verify(cert));
    ...
    /*if (fs.existsSync('./certificate1.p12')) { 
      // delete the file on server after it sends to client
      const unlinkFile = util.promisify(fs.unlink); // to del file from local storage
      await unlinkFile('./certificate1.p12');
    }
    
    fs.writeFileSync("./certificate1.p12",newPkcs12Der,'binary');*/
    ...
    /*const command1 = `openssl genrsa -out aps_development.key 2048`;
    const command2 = `openssl req -new -sha256 -key aps_development.key -out aps_development.csr`;
    const command3 = `openssl x509 -inform DER -outform PEM -in aps_development.cer -out aps_development.pem`;
    const command4 = `openssl pkcs12 -inkey aps_development.key -in aps_development.pem -export -out aps_development.p12`;
    ...
    await execSync(command4, (error, stdout, stderr) => {
        if (error) {
            console.error(`Error creating .p12: ${error.message}`);
            return;
        }
    ...
    //------------------------------------------------------------------------------------
    //------------------------------------------------------------------------------------
    // Paths to your key and certificate

    // Command to create .p12 file
    //const command = `openssl pkcs12 -export -inkey "${privateKeyPath}" -in "${certificatePath}" -out "${outputP12Path}" -password pass:${p12Password}`;
    /*const command = `openssl pkcs12 -export -inkey "${forgeprivateKeyPath}" -in "${certificatePath}" -out "${outputP12Path}"`;
    ...
    await execSync(command, (error, stdout, stderr) => {
        ...
        console.log(`.p12 file created successfully at: ${outputP12Path}`);
    });*/

    //----------------------------------
    //----------------------------------

    /*const pemCert = forge.pki.certificateToPem(certDer);
	  const pemKey = forge.pki.privateKeyToPem(keysForge.privateKey);

    //var newPkcs12Der = forge.pkcs12.toPkcs12Asn1([], '', { algorithm: '3des' });
    var newPkcs12Der = forge.pkcs12.toPkcs12Asn1(pemKey, [pemCert], '', {algorithm: '3des'});*/

    //---------------------------------------------
    //---------------------------------------------

    /*var crt = forge.pki.certificateFromPem(fs.readFileSync("./certificate.crt",'ascii'));
    //var key = forge.pki.privateKeyFromPem(fs.readFileSync("./id_rsa_priv.pem",'ascii'));
    const key = forge.pki.privateKeyToPem(keysForge.privateKey);
    ...
    newCert.publicKey = existingCert.publicKey; // Use the same public key
    ...
    */
    ...
    /*
    // 1. Cargar el certificado y la clave privada desde el sistema de archivos
    const crtPem = fs.readFileSync('./certificate.crt', 'utf8');
    //const keyPem = fs.readFileSync('clave_privada.key', 'utf8');

    // 2. Convertir los PEM a objetos de forge
    const cert = forge.pki.certificateFromPem(crtPem);
    //const privateKey = forge.pki.privateKeyFromPem(keyPem);
    const privateKey = forge.pki.privateKeyToPem(keysForge.privateKey);

    // 3. Crear el almacén PKCS12
    const p12Asn1 = forge.pkcs12.toPkcs12Asn1(privateKey, [cert], 'firma_app', // Contraseña para proteger el archivo .p12
      { algorithm: 'aes256' }
    );

    // 4. Convertir a binario y guardar el archivo
    const p12Der = forge.asn1.toDer(p12Asn1).getBytes();
    fs.writeFileSync('./certificate1.p12', p12Der, 'binary');

    console.log('Archivo .p12 creado con éxito');*/
    ...
    fs.writeFileSync('./certificate1.p12', newPkcs12Der, {encoding: 'binary'});*/
