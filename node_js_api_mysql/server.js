...
app.put('/signPades/:id', (req, res) => {
  ...
  fileUploadLocal(req, res, async () => {
    try {
      ...
      // If client provided a P12, build PKCS#7 (CMS) detached signature server-side and embed
      if (p12file) {
        try {
          const p12Buffer = p12file.data;

          // Parse PKCS#12 to extract private key and certificate using node-forge
          ...
          try {
            ...
            return res.status(400).json({ error: 'Unable to parse PKCS#12. Check password and file.' });
          }
          ...
          if (!privateKey || !certificate) {
            return res.status(400).json({ error: 'Could not extract key or certificate from the PKCS#12 file.' });
          }

          // Create placeholder in PDF
          ...
          // Buffer view for precise offsets
          const pdfBuf = Buffer.from(pdfWithPlaceholder);

          // Locate hex placeholder for signature (/Contents <...>)
          let contentsIdx = pdfBuf.indexOf(Buffer.from('/Contents <'));
          if (contentsIdx === -1) contentsIdx = pdfBuf.indexOf(Buffer.from('/Contents<'));
          if (contentsIdx === -1) return res.status(500).json({ error: 'Could not find /Contents placeholder in PDF' });

          const hexOpenIdx = pdfBuf.indexOf(Buffer.from('<'), contentsIdx);
          const hexCloseIdx = pdfBuf.indexOf(Buffer.from('>'), hexOpenIdx);
          if (hexOpenIdx === -1 || hexCloseIdx === -1) return res.status(500).json({ error: 'Malformed /Contents placeholder' });

          const placeholderHexLength = hexCloseIdx - (hexOpenIdx + 1);

          // Compute ByteRange values (two ranges: before '<' and after '>')
          const start1 = 0;
          const len1 = hexOpenIdx; // bytes up to '<'
          const start2 = hexCloseIdx + 1; // bytes after '>'
          const len2 = pdfBuf.length - start2;

          // Build ByteRange string and replace in-place
          const byteRangeStr = `/ByteRange [${start1} ${len1} ${start2} ${len2}]`;
          const brTag = Buffer.from('/ByteRange [');
          const brIdx = pdfBuf.indexOf(brTag);
          if (brIdx === -1) return res.status(500).json({ error: 'Could not find /ByteRange in PDF' });
          const brEndIdx = pdfBuf.indexOf(Buffer.from(']'), brIdx);
          if (brEndIdx === -1) return res.status(500).json({ error: 'Malformed /ByteRange' });

          const beforeBr = pdfBuf.slice(0, brIdx);
          const afterBr = pdfBuf.slice(brEndIdx + 1);
          const newBrBuf = Buffer.from(byteRangeStr);
          const brSectionLen = brEndIdx - brIdx + 1;
          if (newBrBuf.length > brSectionLen) return res.status(500).json({ error: 'New /ByteRange too long to replace in-place' });
          const paddedBr = Buffer.concat([newBrBuf, Buffer.alloc(brSectionLen - newBrBuf.length, 0x20)]);
          const pdfBufUpdated = Buffer.concat([beforeBr, paddedBr, afterBr]);

          // Create the data to be digested: concatenate the two ranges
          const part1 = pdfBufUpdated.slice(start1, start1 + len1);
          const part2 = pdfBufUpdated.slice(start2, start2 + len2);
          const toDigest = Buffer.concat([part1, part2]);

          // Compute PKCS#7 signedData detached using forge
          const p7 = forge.pkcs7.createSignedData();
          p7.content = forge.util.createBuffer(toDigest.toString('binary'), 'binary');
          ...
          const pkcs7Hex = Buffer.from(raw, 'binary').toString('hex').toUpperCase();

          if (pkcs7Hex.length > placeholderHexLength) return res.status(500).json({ error: 'Generated PKCS#7 exceeds placeholder size' });

          // Insert PKCS#7 hex into placeholder (hex) and pad remaining with zeros
          const paddedPkcs7Hex = pkcs7Hex.padEnd(placeholderHexLength, '0');
          const beforeSig = pdfBufUpdated.slice(0, hexOpenIdx + 1);
          const afterSig = pdfBufUpdated.slice(hexCloseIdx);
          const sigBuf = Buffer.from(paddedPkcs7Hex, 'ascii');
          const finalBuf = Buffer.concat([beforeSig, sigBuf, afterSig]);
          ...
          if (outPath) fs.writeFileSync(outPath, finalBuf);

          return res.json({ status: 200, message: 'Signed PDF created (PKCS#7 embedded)', path: outPath ? outPath : null });
        } catch (err) {
      ...
      if (pkcs7) {
        try {
          ...
          // Work with Buffer to compute exact byte offsets
          const pdfBuf = Buffer.from(pdfWithPlaceholder);

          // Find /ByteRange [ ... ]
          const byteRangeTag = Buffer.from('/ByteRange [');
          const brIdx = pdfBuf.indexOf(byteRangeTag);
          if (brIdx === -1) return res.status(500).json({ error: 'Could not find /ByteRange in PDF' });
          const brEndIdx = pdfBuf.indexOf(Buffer.from(']'), brIdx);
          if (brEndIdx === -1) return res.status(500).json({ error: 'Malformed /ByteRange' });

          // Find /Contents <...>
          const contentsTag1 = Buffer.from('/Contents <');
          let contentsIdx = pdfBuf.indexOf(contentsTag1);
          if (contentsIdx === -1) contentsIdx = pdfBuf.indexOf(Buffer.from('/Contents<'));
          if (contentsIdx === -1) return res.status(500).json({ error: 'Could not find /Contents placeholder in PDF' });
          ...
          const start1 = 0;
          const len1 = hexOpenIdx; // start of '<'
          const start2 = hexCloseIdx + 1; // position after '>'
          const len2 = pdfBuf.length - start2;

          // Build new ByteRange string
          const newByteRange = `/ByteRange [${start1} ${len1} ${start2} ${len2}]`;

          // Replace ByteRange section between brIdx and brEndIdx with new string (pad with spaces if shorter)
          const newBrBuf = Buffer.from(newByteRange);
          const brSectionLen = brEndIdx - brIdx + 1;
          if (newBrBuf.length > brSectionLen) {
            return res.status(500).json({ error: 'New /ByteRange too long to replace in-place' });
          }
          // create a buffer for replaced PDF
          const beforeBr = pdfBuf.slice(0, brIdx);
          const afterBr = pdfBuf.slice(brEndIdx + 1);
          const paddedNewBr = Buffer.concat([newBrBuf, Buffer.alloc(brSectionLen - newBrBuf.length, 0x20)]); // pad with spaces
          let pdfBuf2 = Buffer.concat([beforeBr, paddedNewBr, afterBr]);

          // Now replace the hex contents between hexOpenIdx and hexCloseIdx
          const paddedSigHex = pkcs7Hex.padEnd(placeholderHexLength, '0');
          const sigBuf = Buffer.from(paddedSigHex, 'ascii');

          const beforeSig = pdfBuf2.slice(0, hexOpenIdx + 1);
          const afterSig = pdfBuf2.slice(hexCloseIdx);
          const finalBuf = Buffer.concat([beforeSig, sigBuf, afterSig]);
