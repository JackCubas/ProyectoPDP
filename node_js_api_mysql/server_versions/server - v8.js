...
app.post('/sign/finalize/:id', async (req, res) => {
  ...
  con.connect(async (err) => {
    ...
    // Crear el signer
    //let signerPDF = await new SignPdf();
    ...
    //const signObj = new signer.SignPdf();
      ...
      //const p7 = forge.pkcs12.toPkcs12Asn1([], '', { algorithm: '3des' });
      ...
      console.log(pkcs7Buffer);
      console.log("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ");*/

      //const signedPdfBuffer = signerPDF.sign(modifiedPdfBuffer, pkcs7Buffer);
...
/*var signpdf = require('@signpdf/signpdf').default;
var P12Signer = require('@signpdf/signer-p12').P12Signer;
const signerPH = require('node-signpdf').default;

async function work() {
    // contributing.pdf is the file that is going to be signed
    //var sourcePath = path.join('./contributing.pdf');
    console.log("Entrando en metodo work");
    var pdfBuffer = fs.readFileSync('./contributing1.pdf');
    
    // certificate.p12 is the certificate that is going to be used to sign
    //var certificatePath = path.join('./certificate.p12');
    var certificateBuffer = fs.readFileSync('./certificate.p12');
    ...
    const ByteRange = PDFArrayCustom.withContext(pdfDoc.context);
    ByteRange.push(PDFNumber.of(0));
    ByteRange.push(PDFName.of(signerPH.byteRangePlaceholder));
    ByteRange.push(PDFName.of(signerPH.byteRangePlaceholder));
    ByteRange.push(PDFName.of(signerPH.byteRangePlaceholder));
    ...
    const modifiedPdfBytes = await pdfDoc.save({ useObjectStreams: false });
    const pdfWithPlaceholder = Buffer.from(modifiedPdfBytes);


    if (fs.existsSync('./javascript.pdf')) { 
      // delete the file on server after it sends to client
      const unlinkFile = util.promisify(fs.unlink); // to del file from local storage
      await unlinkFile('./javascript.pdf');
    }
    ...
    signpdf
        ...
        .then(function (signedPdf) {
            // signedPdf is a Buffer of an electronically signed PDF. Store it.
            //var targetPath = path.join(__dirname, '/../output/javascript.pdf');
            fs.writeFileSync('./javascript.pdf', signedPdf);
        })

    console.log("Saliendo del metodo work");
}*/
