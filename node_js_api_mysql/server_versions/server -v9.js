...
app.post('/sign/finalize/:id', async (req, res) => {
  ...
  con.connect(async (err) => {
    ...
    //var keys = genKeyPairStrong()
    const certDecode = forge.util.decode64(certificate);
    const certDer = "-----BEGIN CERTIFICATE-----" + "\n" + certificate + "\n" + "-----END CERTIFICATE-----";


    //------------------------------------------------------------------------------------
    /*var cleanedCert = certDecode.replace(/[^a-zA-Z0-9\s]/g, '-');
    //var cleanedCert = cleanedCert.replace("♂","-").replace("♀","-");
    const regexp = /(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;
    var urlCert = certDecode.match(regexp)
    //const certDer = forge.pki.createCertificate();
    ...
    //const altNameIPs = ["127.0.0.1", "127.0.0.2"];

    //certDer.setSubject(attrs);
    //certDer.setIssuer(attrs);

    // add alt names so that the browser won't complain
    //certDer.setExtensions([{
    //  name: 'subjectAltName',
    //  altNames: []
    //}]);

    certDer.publicKey = keys.publicKey;

    // NOTE: serialNumber is the hex encoded value of an ASN.1 INTEGER.
    // Conforming CAs should ensure serialNumber is:
    // - no more than 20 octets
    // - non-negative (prefix a '00' if your value starts with a '1' bit)
    certDer.serialNumber = '01' + crypto.randomBytes(19).toString("hex"); // 1 octet = 8 bits = 1 byte = 2 hex chars
    certDer.validity.notBefore = new Date();
    certDer.validity.notAfter = new Date(new Date().getTime() + 1000 * 60 * 60 * 24 * 2);
    
    // self-sign certificate
    //certDer.sign(keys.privateKey);
    ...
    //var certFromPem = forge.pki.certificateFromPem(certDer);

    /*certDer.publicKey = keys.publicKey;
    certDer.serialNumber = '01';
    certDer.notBefore = new Date();
    certDer.notAfter = new Date();
    ...
    var newPkcs12Der = forge.pkcs12.toPkcs12Asn1(pemKey, [pemCert], '', 
    {generateLocalKeyId: true, friendlyName: 'test', algorithm: '3des'});
    

    if (fs.existsSync('./certificate.p12')) { 
      // delete the file on server after it sends to client
      const unlinkFile = util.promisify(fs.unlink); // to del file from local storage
      await unlinkFile('./certificate.p12');
    }

    fs.writeFileSync('./certificate.p12', newPkcs12Der, {encoding: 'binary'});
